...
# importing modules
from psychopy import core, event, visual, gui, logging, parallel
...
while already_exists:
    ...
    learn_file_name         = my_directory     + "/data/exp/Learningtwo_participant_"          + str(info["Participant number"]) + ".csv"
...
if iEEG == 1:
    ...
    s.write(bytes([0]))
datetime.now()
logging.info("Learning-2: started")
...
# set the exptwo trials
nexpone         = 35
nexptwo         = 35


#set the stimulus parameters for the Swahili options
...
# load the stimulus for the experiment
all_faces       = nexpone + nexptwo
list_words      =  pandas.read_csv(exptwo_file_name, sep = ",", header = None, names = ["s"])
exptwo_words      = list_words.s[:]
...
df["Swahili1"]   = exptwo_words.iloc[nexptwo*0:nexptwo*1].reset_index(drop=True)
df["Swahili2"]   = exptwo_words.iloc[nexptwo*1:nexptwo*2].reset_index(drop=True)
df["Swahili3"]   = exptwo_words.iloc[nexptwo*2:nexptwo*3].reset_index(drop=True)
df["Swahili4"]   = exptwo_words.iloc[nexptwo*3:nexptwo*4].reset_index(drop=True)
print(dataset)
df["Face"]       = dataset.Pictures.iloc[6+nexpone:nexptwo+nexpone+6].reset_index(drop=True)
print(df["Face"])
...
for j in range(nexptwo):
    ...
    #add a marker in the reading phase
   ...
   # 
    ...
    #add marker in the encoding phase 
    if iEEG == 1:
        s.write(bytes([3]))
        time.sleep(marker_delay)
        s.write(bytes([0]))
