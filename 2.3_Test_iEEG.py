...
# revized by zhaoxiaohui for iEEG experiment , spring 2021
...
while already_exists or already_exists_train or unknown_participant  or unknown_participant_train:
    ...
    rec_file_name       = recognitionDir + "/Recognition_participant_"           + str(info["Participant number"]) + ".csv"
    rec_train_file_name = recognitionDir + "/Recognition_training_participant_"  + str(info["Participant number"]) + ".csv"
    exp_file_name       = my_directory + "/data/exp/Learning_participant_"                      + str(info["Participant number"]) + ".csv"
...
datetime.now()
logging.info("Test1-train: started")
...
for j in range(ntrials):
    ...
    datetime.now()
    logging.info("Anticipation_test")
